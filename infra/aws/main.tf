# Fase 1: storage propio del catálogo entity360 (regla 03, D7/D8 del spike).
# Unity Catalog asume el rol entity360-uc para leer y escribir en el bucket.

data "aws_caller_identity" "current" {}

locals {
  account_id  = data.aws_caller_identity.current.account_id
  bucket_name = "entity360-uc-${local.account_id}"
  role_name   = "entity360-uc"
  role_arn    = "arn:aws:iam::${local.account_id}:role/${local.role_name}"
}

# Sin force_destroy: el destroy falla si el bucket tiene datos (se vacía a propósito,
# ver docs/destroy.md).
...
# que pueda asumirse a sí mismo). El self-trust va por condición sobre la cuenta porque IAM
# rechaza como principal un rol que todavía no existe.
...
resource "aws_iam_role" "uc" {
  ...
  description        = "Unity Catalog (Free Edition) -> storage propio del catalogo entity360"
}
